...
geometry_msgs::PoseStamped get_rviz_ref_posistion(const prometheus_msgs::ControlCommand& cmd)
{
    ...
    else if(cmd.Mode == prometheus_msgs::ControlCommand::Move)
    {
        if( cmd.Reference_State.Move_mode  == prometheus_msgs::PositionReference::XYZ_POS )
        {
            ...
            random[0] = prometheus_control_utils::random_num1();
            random[1] = prometheus_control_utils::random_num1();
            random[2] = prometheus_control_utils::random_num1();
            ref_pose.pose.position.x = cmd.Reference_State.position_ref[0];
            ref_pose.pose.position.y = cmd.Reference_State.position_ref[1];
            ref_pose.pose.position.z = cmd.Reference_State.position_ref[2];
            ref_pose.pose.orientation.x =cmd.Reference_State.position_ref[0] - _DroneState.position[0] + random[0];
            ref_pose.pose.orientation.y =cmd.Reference_State.position_ref[1] - _DroneState.position[1]+ random[1] + 0.06;
            ref_pose.pose.orientation.z =cmd.Reference_State.position_ref[2] - _DroneState.position[2]+ random[2] - 0.1;
        ...
        {
            Eigen::Vector3d random;
            random[0] = prometheus_control_utils::random_num(0.005,0.3);
            random[1] = prometheus_control_utils::random_num(0.005,0.4);
            random[2] = prometheus_control_utils::random_num(0.005,0.1);
            ref_pose.pose.position.x = cmd.Reference_State.position_ref[0];
            ref_pose.pose.position.y = cmd.Reference_State.position_ref[1];
            ref_pose.pose.position.z = _DroneState.position[2] + cmd.Reference_State.velocity_ref[2] * dt;
            ref_pose.pose.orientation = _DroneState.attitude_q;
        ...
        ref_pose.pose.position.y = cmd.Reference_State.position_ref[1];
        ref_pose.pose.position.z = cmd.Reference_State.position_ref[2];
        ref_pose.pose.orientation.x =cmd.Reference_State.position_ref[0] - _DroneState.position[0];
        ref_pose.pose.orientation.y =cmd.Reference_State.position_ref[1] - _DroneState.position[1];
        ref_pose.pose.orientation.z =cmd.Reference_State.position_ref[2] - _DroneState.position[2];
        ref_pose.pose.orientation.w = 0;
        }
